...
2. Start the Archicad-side bridge that implements the live adapter contract.
3. Set `CCP_ARCHICAD_ADAPTER=live`, `ARCHICAD_HOST`, and `ARCHICAD_PORT`.
4. Apply an approved CCP property update through the outbound connector flow.
5. Inspect the target object properties in Archicad.
6. Verify schedules can display the updated value.
7. Verify a Graphic Override rule can react to the updated value.
