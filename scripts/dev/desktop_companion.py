...
class CompanionState:
    ...
    def snapshot_summary(self) -> dict[str, int] | None:
        try:
            snapshot = self._request_bridge("snapshot")
        except Exception:
            return None
        ...
        if not isinstance(zones, list) or not isinstance(elements, list):
            return None
        return {"zones": len(zones), "elements": len(elements)}
    ...
    class CompanionHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            if not self.require_token():
                return
            if self.path.startswith("/companion/status"):
                self.send_json(200, state.status())
                return
            if self.path.startswith("/companion/logs"):
                limit = 100
                if "?" in self.path:
        ...
        def do_POST(self) -> None:
            if not self.require_token():
                return
            try:
                if self.path == "/companion/connect":
                    self.send_json(200, state.connect())
                    return
                if self.path == "/companion/disconnect":
                    self.send_json(200, state.disconnect())
                    return
                if self.path == "/companion/inbound":
                    self.send_json(200, state.run_connector("inbound"))
                    return
                if self.path == "/companion/outbound":
